...
Genfeed Core includes the API only. To run the full Studio interface from the workspace monorepo:

```bash
# In a separate terminal, from the `cloud/` workspace root
cd cloud/apps/web/studio
bun install
bun run dev
```

The Studio will be available at `http://localhost:3106`.
